...
#echo isset($_POST['submit2']);
  $p=$_SESSION['id'];
   $sl="SELECT * FROM takes where instructor_id='$p'";
 $result=pg_query($connect_str,$sl);
$ex=1;
  while ($row=pg_fetch_row($result)) {
    ...
    if($sub){
      ...
      #$rw=pg_query($query);
      $query="UPDATE section set schedule='$w' WHERE year='$row[0]' and name='$row[1]'; ";
      $rw=pg_query($query);

      $sql="SELECT id,hours FROM course where year='$row[0]'";
       $rt=pg_query($connect_str,$sql);
       while ($fet=pg_fetch_row($rt)) {
      $query="UPDATE takes set hour='$fet[1]' where year='$row[0]' and section='$row[1]' and  course_id='$fet[0]'; ";
        $rw=pg_query($query);
           }
    ...
    $qy="select schedule from teacher where id='$p'";
    #+echo $q;
    $rw=pg_query($connect_str,$qy);
    while($ew=pg_fetch_row($rw)){
        $myu=json_decode($ew[0]);
    
      for ($g=0; $g <5 ; $g++) {
      $coun=0;
      foreach ($myu[$g] as $key) {#echo "$key";
      $eru=json_decode($key);
      echo "$eru[1],asd";
      #if("as"=="as"){echo "gh";}
        if($eru!='empty' and $row[0]==$eru[0] and $row[1]==$eru[1]){
          $myu[$g][$coun]='empty';
          #echo "string";
        }
        $coun++;
      ...
      $myu=json_encode($myu);
      $qu="update teacher set schedule='$myu' where id='$p'; ";
    $rw=pg_query($qu);
...
if(isset($_POST['submit1']))
  ...
  echo "<form action='' method='post'>" ;
   ...
   while ($row=pg_fetch_row($result)) {
    ...
    echo "instuctor_id:<label><td><input type='number'  name=$num value='' ></td></label>";
    $num++;
    echo "<br>";
echo "<br>";
